...
var mainworker = new Worker("bin/mainworker_plus.js");

var walletworkerList = null;
var createwalletworkerList = function(){
  if (walletworkerList)
    return;
  walletworkerList = [];
  let cores = 4;
...
var find_least_busy_worker_id = function(){
    let worker_id = 0;
...
var http_server_calls = new Map();
var last_call_handle = 0;

var cn_walletd_start = function(argv){ // restarting walletd cancels all outstanding http requests
	var transferObject = {
		'func': "walletd_start",
		'argv':argv
	};
...
var cn_http_server_call = function(method, uri, body, cb){ // (string, string, string, function(int, string))
	last_call_handle += 1;
